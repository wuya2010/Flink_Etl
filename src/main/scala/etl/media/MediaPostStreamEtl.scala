...
object MediaPostStreamEtl extends FlinkEnv {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    //从kafka读取数据
    //    val kafka_gofish_media: FlinkKafkaConsumer011[gofish_media] =
    //      new FlinkKafkaConsumer011[gofish_media]("", new gofishMediaDeserializationSchema, properties)
    //
    //    val gofish_media_stream = env.addSource(kafka_gofish_media)
    //
    //    //获取样例类里面的字段
    //    val getStream = gofish_media_stream.map(media => {
    //      val message = media.name
    //      gofish_media(message)
    //    })


    //es原表：dw_gofish_media_post

    //将数据写入es
    val inputStream = env.readTextFile("E:\\Project\\Flink_datawork\\src\\main\\scala\\etl\\media\\media.txt")

    //字段清洗
    val etl_media_stream = inputStream.map(str => {
      val mediaArr= str.split("&")
      media_post(mediaArr(0),mediaArr(1),mediaArr(2).toLong,etl_word(mediaArr(3)))
    })
...
//    result.addSink((new MedisEsSink).esBuilder.build())
  ...
  case class media_post(
                         post_user_name:String,`type`:String ,timeStamp:Long,post_content: String
                       )

  case class media_post_industry(
                                  post_user_name:String,`type`:String ,post_content: String,sub_industry_id: mutable.Set[String]
                                )

  case class media_post_result(
                                post_user_name:String,`type`:String ,post_content: String,sub_industry_id: String, parent_id:String
                              )
