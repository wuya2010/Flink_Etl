...
import HbseUtil.HbaseSink
import com.alibaba.fastjson.JSONObject
import kafkaUtil.{DwdKafkaProducerSerializationSchema, TopicAndValueDeserializationSchema, modleDeserializationSchema}
...
//    largest : 自动把offset设为最大的offset；
    ...
    //将message内容字符串获取
    val kakfaStream = env.addSource(new FlinkKafkaConsumer011[String]("", new SimpleStringSchema, properties))
...
//      .assignAscendingTimestamps()//增量时间戳
    ...
    val kakfaStreammodle = env.addSource(new FlinkKafkaConsumer011[modle]("testTopic", new modleDeserializationSchema, properties))
    ...
    val kafkaTopic = new FlinkKafkaConsumer011[TopicAndValue](topicList, new TopicAndValueDeserializationSchema, properties)
    ...
    resultStream.addSink(
      new FlinkKafkaProducer011[TopicAndValue]("101.36.109.223:9092,101.36.109.7:9092,101.36.109.94:9092","",new DwdKafkaProducerSerializationSchema))//自定义：序列化类
