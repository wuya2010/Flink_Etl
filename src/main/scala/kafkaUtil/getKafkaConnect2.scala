...
//    val kakfaStreammodle = env.addSource(new FlinkKafkaConsumer011[modle]("testTopic", new modleDeserializationSchema, properties))
    ...
    val kakfaStreammodle = env.addSource(new FlinkKafkaConsumer011[String]("testTopic", new SimpleStringSchema, properties))


    //自定flink 消费kafka的偏移量位置
    val consumer = new FlinkKafkaConsumer011[String]("testTopic", new SimpleStringSchema(), properties)
